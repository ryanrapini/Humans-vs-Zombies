...
if(isset($_POST[firstname])) {
	$settings = include('../credentials.php');
	$con = mysql_connect($settings['database']['host'],$settings['database']['user'],$settings['database']['pass']);
	if (!$con)
	  {
	...
	$gender = ereg_replace("[^A-Za-z0-9]", "", $_POST[gender] );
	if ($gender != "female"){
	$gender = "male";
	}
	...
	mysql_select_db($settings['database']['db'], $con);
